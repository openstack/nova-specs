...
Note that this spec will only cover the high level changes to the API and
...
.. note::
   ...
   The following ``hw_ephemeral_encryption`` image properties do not relate to if
...
Introduce new compatibility traits
----------------------------------

A new ``COMPUTE_EPHEMERAL_ENCRYPTION`` compute compatibility trait will be
...
supports using the ``[ephemeral_storage_encryption]/default_format``
configurable.

New ``COMPUTE_EPHEMERAL_ENCRYPTION_$FORMAT`` compute compatibility traits will
be added to os-traits and reported by virt drivers to indicate support for
specific ephemeral storage encryption formats. For example:
...
* Introduce ``COMPUTE_EPHEMERAL_ENCRYPTION`` and
  ``COMPUTE_EPHEMERAL_ENCRYPTION_$FORMAT`` compatibility traits
